...
let ``remove deletes key`` () =
    let d = BinaryDict.empty<int,string>() |> BinaryDict.add 2 "two" |> BinaryDict.add 1 "one" |> BinaryDict.add 3 "three"
...
let ``mapValues transforms values and preserves keys`` (xs: list<int*int>, f: int->int) =
    ...
    keys1 = keys2
    // values transformed
    && (keys1 |> List.forall (fun k -> BinaryDict.tryFind k d2 = (BinaryDict.tryFind k d |> Option.map f)))
...
let ``filter by even keys keeps subset`` (xs: list<int*int>) =
    ...
    let expected = xs |> Seq.choose (fun (k,_) -> if k % 2 = 0 then Some k else None) |> Set.ofSeq
    keys2 = expected
